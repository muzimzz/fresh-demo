...
실수로 엔티티를 통째로 로그에 넘기더라도 사고로 이어지지 않도록, `Member`/`Admin`/`Address`의
`toString()`을 오버라이드해서 최소 필드만(비밀번호 해시·토큰 해시 등은 아예 제외) 노출하게
만들어뒀다. 이건 마지막 방어선이고, 기본 원칙은 여전히 "애초에 엔티티를 로그에 넘기지 않는다"이다.
