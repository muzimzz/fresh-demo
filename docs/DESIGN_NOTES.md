...
- ✅ **Redis 장애 대비 DB 백업 계층.** `RefreshTokenBackup`(MySQL) 엔티티에 `save`/`find`/`delete`/`compareAndSave` 전부 write-through로 같이 기록. Redis 호출이 "정상적으로 값이 없음"이 아니라 "연결 자체가 안 됨"(`DataAccessException`)일 때만 DB로 폴백— CAS도 Redis가 죽으면 DB의 조건부 `UPDATE`(영향받은 row 수로 성공 판단)로 대체. Redis 이중화(Sentinel/Cluster)는 별도로 구성하지 않기로 하고, 이 write-through+폴백 계층으로 대신하기로 결정.
...
- ✅ **refreshToken은 원문이 아니라 SHA-256 해시로 저장(`TokenHasher`).** Redis value / `RefreshTokenBackup.tokenHash` 둘 다 해시만 저장 — 저장소가 유출돼도 그 값을 그대로 제시해서 로그인할 수 없다. 서버가 원문을 다시 복원할 필요가 없는(클라이언트 쿠키의 원문과 "일치 여부"만 확인하면 되는) 값이라 암호화가 아니라 해싱을 썼다(비밀번호와 같은 이유). 비밀번호(bcrypt)와 달리 salt/느린 해시는 안 씀 — JWT는 서명 자체가 고엔트로피라 브루트포스 방어 목적의 느린 해시가 필요 없고, 매 요청마다 검증해야 해서 오히려 빨라야 함. 죽은 코드였던 `find()`(저장된 원문을 그대로 반환)는 해싱 방식과 양립할 수 없어 `matches(role, id, candidateToken)`(해시 비교 후 boolean 반환)로 교체.
...
- ✅ **엔티티 `toString()` 마스킹 오버라이드.** 위 원칙에도 불구하고 실수로 `log.info("{}", member)`처럼 엔티티 객체 자체를 로그에 넘기는 실수를 막는 마지막 방어선으로, `Member`/`Admin`/`Address`/`RefreshTokenBackup`의 `toString()`을 오버라이드했다. `Admin.passwordHash`, `RefreshTokenBackup.token`(refreshToken 원문)처럼 유출되면 바로 사고로 이어지는 값은 아예 필드 자체를 뺐고, email/name처럼 식별에 필요한 값은 `PiiMasker`로 부분 마스킹만 해서 남겼다.
...
- ✅ **스케줄러 공통 로깅(`SchedulerLoggingAspect`).** `@Scheduled` 메서드 실행마다 시작/종료(또는 실패)/소요시간을 AOP로 자동 로깅(`spring-boot-starter-aop` 추가). `RefreshTokenCleanupScheduler`가 "삭제 건수 0이면 로그를 아예 안 남기는" 방식이라 "이 시간에 스케줄러가 정상적으로 돌긴 한 건지" 확인이 안 되던 구멍을, 스케줄러 코드를 안 건드리고 공통으로 해결. 새 스케줄러가 추가돼도 자동 적용됨.
